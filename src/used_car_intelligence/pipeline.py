...
def save_figures(df: pd.DataFrame, model_results: dict, importance: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig.tight_layout(); fig.savefig(output_dir / "median_price_by_brand.png", dpi=180); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(output_dir / "price_vs_mileage.png", dpi=180); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(output_dir / "depreciation_curve.png", dpi=180); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(output_dir / "model_mae.png", dpi=180); plt.close(fig)
    ...
    fig.tight_layout(); fig.savefig(output_dir / "feature_importance.png", dpi=180); plt.close(fig)
...
def summarize(df: pd.DataFrame, quality: DataQuality, model_results: dict, importance: pd.DataFrame) -> dict:
    brand_stats = (
        ...
        .agg(listings=("price", "size"), median_price=("price", "median"), median_mileage=("mileage", "median"), median_mpg=("mpg", "median"))
...
def run(raw_path: Path, project_root: Path) -> dict:
    ...
    processed = project_root / "data" / "processed" / "used_cars_clean.csv"
    clean.to_csv(processed, index=False)
    write_sqlite(clean, project_root / "data" / "processed" / "used_cars.sqlite")
    model_results, rf_model, predictions = train_models(clean)
    predictions.to_csv(project_root / "reports" / "model_predictions.csv", index=False)
    importance = feature_importance(rf_model)
    importance.to_csv(project_root / "reports" / "feature_importance.csv", index=False)
    save_figures(clean, model_results, importance, project_root / "reports" / "figures")
